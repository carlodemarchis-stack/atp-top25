#!/usr/bin/env python3
"""Live-refresh the WTA US Open tree: /draw positions + /matches results propagated up.
Handles any completion level (0 played -> all pending; final played -> champion)."""
...
def draw_positions(gid, year):
    ...
    lines = sorted([L for L in (sing.get("Draw") or {}).get("DrawLine") or [] if isinstance(L, dict)],
...
def matches_index(gid, year):
    ...
    for m in ms:
        ...
        idx[frozenset([idA, idB])] = {"played": m.get("MatchState")=="F",
            "winId": idA if w=="A" else idB, "sc": {idA: scA, idB: scB}, "sched": sched}
    return idx

def fmt_sched(rec):
    s = rec["sched"]; date = ""
    try:
        dt = datetime.datetime.fromisoformat(s["ts"]); date = f"{dt.strftime('%a')} {dt.day} {dt.strftime('%b')}"
...
def build_live(gid, year):
    ...
    def apply_sched(node):                          # upcoming, scheduled -> "Sat 30 Aug · Starting at 11:00 AM · Grandstand"
        ps = [x for x in node["p"] if x.get("id")]
        if len(ps) != 2: return
        rec = midx.get(frozenset([ps[0]["id"], ps[1]["id"]]))
        if rec and not rec["played"] and rec.get("sched"): node["up"] = fmt_sched(rec)
    def winner_of(node):
        for x in node.get("p", []):
    ...
    def build(lo, size, r):
        if r == 0:
            node = {"r":0,"dur":"","p":[player(pos[lo]), player(pos[lo+1])]}; apply_result(node); apply_sched(node); return node
        half = size//2
        left = build(lo, half, r-1); right = build(lo+half, half, r-1)
        lw, rw = winner_of(left), winner_of(right)
        node = {"r":r,"dur":"","p":[player(lw) if lw else {"tbd":True}, player(rw) if rw else {"tbd":True}],
                "c":[left, right]}
        if lw and rw: apply_result(node); apply_sched(node)
        return node
    return build(0, N, maxR), countries

tree, countries = build_live(905, 2026)
def count_played(n, acc=[0]):
    if n.get("p") and any(p.get("w") for p in n["p"]): acc[0]+=1
    for c in n.get("c",[]): count_played(c, acc)
    return acc[0]
champ = next((p["n"] for p in tree["p"] if p.get("w")), None)
print("WTA US Open live — decided matches:", count_played(tree), "| champion:", champ, "| countries:", len(countries))
...
open(F, "w").write(h); print("spliced | bytes", len(h))
